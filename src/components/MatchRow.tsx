    ...
    <div className="bg-[#242424] hover:bg-[#2d2d2d] p-3 flex items-center gap-3 border-b border-[#1a1a1a] transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-center mb-0.5">
          <span
            className={`text-xs font-bold truncate ${match.score.home > match.score.away ? "text-white" : "text-gray-400"}`}
          >
            {match.home}
          </span>
          <span className="text-yellow-500 font-mono font-black text-xs">
            {match.score.home}
          </span>
        </div>
        <div className="flex justify-between items-center">
          <span
            className={`text-xs font-bold truncate ${match.score.away > match.score.home ? "text-white" : "text-gray-400"}`}
          >
            {match.away}
          </span>
          <span className="text-yellow-500 font-mono font-black text-xs">
